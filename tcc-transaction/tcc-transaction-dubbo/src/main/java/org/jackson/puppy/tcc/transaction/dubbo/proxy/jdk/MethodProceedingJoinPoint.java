...
public class MethodProceedingJoinPoint implements ProceedingJoinPoint, JoinPoint.StaticPart {
	...
	public Object proceed() throws Throwable {
		...
		try {
			...
			// Invoked method threw a checked org.jackson.puppy.redis.lock.redis.exception.
			// We must rethrow it. The client won't see the interceptor.
			throw ex.getTargetException();
		} catch (IllegalArgumentException ex) {
			throw new TccException("Tried calling method [" +
					method + "] on target [" + target + "] failed", ex);
		} catch (IllegalAccessException ex) {
			throw new TccException("Could not access method [" + method + "]", ex);
		}
	}
