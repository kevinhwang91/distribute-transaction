...
--
-- Table structure for table `tcc_transaction_order`
--
...
) ENGINE=InnoDB AUTO_INCREMENT=81510 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
begin
	...
	while i < acc_quantity do
		while j < order_quantity_per_acc do
			...
			set j = j + 1;
            set k = k + 1;
			insert into `order`(order_number, account_number, status, updated_date) values(order_number, account_number, status, current_timestamp);
		end while;
        set j = 0;
		set i = i + 1;
	end while;
